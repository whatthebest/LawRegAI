...
/** Compute the highest index seen either in the counter or in existing sops */
async function computeBaseIndex() {
  ...
  // Read /meta/sopCounter (safe – ignore errors instead of throwing)
  try {
  ...
  // Look at the highest sopIndex inside /sops (safe – ignore errors)
  try {
...
/** PUT: sync /meta/sopCounter to the highest sopIndex in DB (optional utility) */
